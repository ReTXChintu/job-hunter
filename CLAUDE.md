...
- `apps/backend` is a self-hosted Node/Fastify service, wire-compatible with `job-hunter-relay`'s accounts/devices/pairing/WebSocket protocol, plus a generic account-scoped data API (`/v1/data/:collection`) and joined application read views (`/v1/applications`). It is built and tested but **not yet wired into the desktop or mobile app** -- both still use `job-hunter-relay` as of this writing. See `docs/backend.md` for what's done and the remaining integration phases before touching `crates/job-hunter-core/src/store/`.
